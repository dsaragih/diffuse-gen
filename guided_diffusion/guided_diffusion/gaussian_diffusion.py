...
class GaussianDiffusion:
    ...
    def p_sample(
        self, model, x, t, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, pred_xstart=None
    ):
        """
        ...
        """
        if pred_xstart is not None and model_kwargs["init_image"] is not None:
            # model_kwargs["init_image"] shape: (B, C, W, H), range [-1, 1]
            init_img, init_img_mask = model_kwargs["init_image"], model_kwargs["init_image"][:, 3, ...].unsqueeze(1)
            # print("init_img_mask shape: ", init_img_mask.shape)
            # print("init_img min: ", init_img.min(), "max: ", init_img.max())
            # print("init_img_mask min: ", init_img_mask.min(), "max: ", init_img_mask.max())
            # alpha_cumprod = _extract_into_tensor(
            #     self.alphas_cumprod, t, x.shape)

            # img_weight = th.sqrt(alpha_cumprod)
            # img_part = img_weight * init_img

            # noise_weight = th.sqrt((1 - alpha_cumprod))
            # noise_part = noise_weight * th.randn_like(x)

            # weighed_gt = img_part + noise_part

            weighed_gt = self.q_sample(init_img, t)
            #import matplotlib.pyplot as plt

            # plt.imshow(init_img[0].permute(1, 2, 0)[..., :3].detach().cpu().numpy())
            # plt.savefig("gt.png")
            # plt.imshow(init_img_mask[0].detach().cpu().numpy(), cmap="gray")
            # plt.savefig("mask.png")
            init_img, init_img_mask = (init_img + 1) / 2, (init_img_mask + 1) / 2 # range [0, 1]
            img_known = (1-init_img_mask) * weighed_gt
            img_unknown = (init_img_mask) * x # mask inverted
            # plt.imshow(img_known[0].permute(1, 2, 0)[..., :3].detach().cpu().numpy())
            # plt.savefig("known.png")
            # plt.imshow(img_unknown[0].permute(1, 2, 0)[..., :3].detach().cpu().numpy())
            # plt.savefig("unknown.png")
            x = img_known + img_unknown
            #x = x * 2 - 1
            # plt.imshow(test_x[0].permute(1, 2, 0)[..., :3].detach().cpu().numpy())
    ...
    def p_sample_loop(
        ...
        if final_only:
            ...
            return list(self.p_sample_loop_progressive(
            model,
            shape,
            noise=noise,
            clip_denoised=clip_denoised,
    ...
    def p_sample_loop_progressive(
        ...
        # If init_image batch < batch_size, tile it
        init_image_batch = init_image
        if init_image.shape[0] < batch_size:
            init_image_batch = th.tile(init_image, dims=(batch_size, 1, 1, 1))

        if inpainting:
            # Code adopted from RePaint
            ...
            model_kwargs["init_image"] = init_image_batch
            ...
            for t_last, t_cur in time_pairs:
                idx_wall += 1
                t_last_t = th.tensor([t_last] * batch_size,
                                     device=device)
            ...
            # init_image when styling is different than init_image when inpainting
            img = self.q_sample(
...
#                 img = out["sample"]
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if noise is None:
            noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start.float(), t, noise=noise)
